...
fn dispatch_multiplexer<T: Effect, const N: usize>(ptr: *mut u8, event: &Event) {
...
impl<'a, T: Effect, const N: usize> Effect for MultiplexerEffect<'a, T, N> {
    ...
    fn dispatch(&mut self, event: &Event) {
        match event {
            Event::MuxEffect(MultiplexerEffectEvent::Bypass(bypass)) => {
                self.bypass = *bypass;
            }
            Event::MuxEffect(MultiplexerEffectEvent::SetDryWet(index, wet)) => {
                if *index < N {
...
fn dispatch_multiplexer1<T1: Effect>(ptr: *mut u8, event: &Event) {
...
impl<'a, T1: Effect> Effect for Multiplexer1Effect<'a, T1> {
    ...
    fn dispatch(&mut self, event: &Event) {
        match event {
            ...
            Event::MuxEffect(MultiplexerEffectEvent::SetDryWet(0, wet)) => {
...
fn dispatch_multiplexer2<T1: Effect, T2: Effect>(ptr: *mut u8, event: &Event) {
...
impl<'a, T1: Effect, T2: Effect> Effect for Multiplexer2Effect<'a, T1, T2> {
    ...
    fn dispatch(&mut self, event: &Event) {
        match event {
            ...
            Event::MuxEffect(MultiplexerEffectEvent::SetDryWet(1, wet)) => {
...
fn dispatch_multiplexer3<T1: Effect, T2: Effect, T3: Effect>(ptr: *mut u8, event: &Event) {
...
impl<'a, T1: Effect, T2: Effect, T3: Effect> Effect for Multiplexer3Effect<'a, T1, T2, T3> {
    ...
    fn dispatch(&mut self, event: &Event) {
        match event {
            ...
            Event::MuxEffect(MultiplexerEffectEvent::SetDryWet(2, wet)) => {
    ...
    for Multiplexer4Effect<'a, T1, T2, T3, T4>
{
    ...
    fn dispatch(&mut self, event: &Event) {
        match event {
            ...
            Event::MuxEffect(MultiplexerEffectEvent::SetDryWet(3, wet)) => {
