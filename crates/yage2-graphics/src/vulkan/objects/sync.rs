use crate::vulkan::{VkObject, GraphicsError};
...
impl Semaphore {
    pub fn new(device: &Device, name: Option<String>) -> Result<Self, GraphicsError> {
        debug!("Creating semaphore: {:?}", name);
        let semaphore_create_info = vk::SemaphoreCreateInfo::default();
        let vk_semaphore = unsafe {
            device
                .create_semaphore(&semaphore_create_info, None)
                .map_err(GraphicsError::SemaphoreCreateFailed)?
        };
    ...
    pub fn new(device: &Device, name: Option<String>) -> Result<Self, GraphicsError> {
        ...
        let vk_fence = unsafe {
            device
                ...
                .map_err(GraphicsError::FenceCreateFailed)?
        };
        Ok(Fence { vk_fence, name })
    }

    pub fn reset(&self, device: &Device) -> Result<(), GraphicsError> {
        unsafe {
            device
                .reset_fences(&[self.vk_fence])
                .map_err(GraphicsError::FenceResetFailed)?;
        }
        Ok(())
    }

    pub fn wait(&self, device: &Device, timeout: u64) -> Result<(), GraphicsError> {
        unsafe {
            device
                .wait_for_fences(&[self.vk_fence], true, timeout)
                .map_err(GraphicsError::FenceWaitFailed)?;
        }
        Ok(())
    }
}
