...
use crate::vulkan::objects::{
    get_required_instance_extensions, get_required_layers, get_wanted_instance_extensions,
    get_wanted_layers,
};
use crate::vulkan::{GraphicsError, DEBUG_REPORT_EXTENSION_NAME, DEBUG_UTILS_EXTENSION_NAME};
use ash::vk;
...
unsafe fn get_supported_instance_extensions(
    entry: &ash::Entry,
) -> Result<Vec<*const c_char>, GraphicsError> {
    let extensions = entry
        .enumerate_instance_extension_properties(None)
        .map_err(GraphicsError::EnumerateExtensionsError)?;
...
unsafe fn get_supported_layers(entry: &ash::Entry) -> Result<Vec<*const c_char>, GraphicsError> {
    let layers = entry
        .enumerate_instance_layer_properties()
        .map_err(GraphicsError::EnumerateLayersError)?;
...
pub unsafe fn get_layers(entry: &ash::Entry) -> Result<Vec<*const c_char>, GraphicsError> {
    let mut required = get_required_layers().into_iter().collect::<Vec<_>>();
...
) -> Result<(vk::DebugReportCallbackEXT, vk::DebugUtilsMessengerEXT), GraphicsError> {
    ...
    if cfg!(debug_assertions) {
        if contains(instance_extensions, DEBUG_REPORT_EXTENSION_NAME) {
            ...
            debug_report_callback = debug_report_loader
                ...
                .map_err(GraphicsError::CreateDebugReportFailed)?;
            ...
            debug_messenger = debug_utils_loader
                ...
                .map_err(GraphicsError::CreateDebugMessengerFailed)?;
        } else {
