...
impl UserAssetFile {
    pub fn convert(
        ...
        let irs = match &self.asset.properties {
            ...
            UserAssetProperties::Texture2D(texture) => {
                convert_texture2d(self, cache_dir, cwd, texture)
            }
            UserAssetProperties::TextureCube(cube) => {
                convert_texture_cube(self, cache_dir, cwd, cube)
            }
